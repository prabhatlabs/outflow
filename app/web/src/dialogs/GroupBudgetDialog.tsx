...
export function GroupBudgetDialog({
  ...
  id: string
  payload?: DialogPayloadMap["groupBudget"]
}) {
  const groupId = payload?.groupId ?? ""
  const close = useDialogStore((s) => s.close)
  const create = useGroupBudgetsStore((s) => s.create)
  const edit = useGroupBudgetsStore((s) => s.edit)
  const items = useGroupBudgetsStore((s) => s.items)

  const existing = useMemo(
    () =>
      payload?.budgetId
        ? (items.find((b) => b.id === payload.budgetId) ?? null)
        : null,
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>{isEdit ? "Edit budget" : "New group budget"}</DialogTitle>
        <DialogDescription>
      ...
      <div className="grid gap-3">
        <div className="grid gap-1.5">
          <Label htmlFor="group-budget-amount">Amount</Label>
          <Input
            id="group-budget-amount"
            type="number"
            min={0}
            step="0.01"
            value={amount}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="group-budget-threshold">Alert at %</Label>
            <Input
              id="group-budget-threshold"
              type="number"
              min={1}
              max={100}
              value={threshold}
